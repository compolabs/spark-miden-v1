...
/* #[test]
fn test_fuzz_partial_swap_fill() {
    let mut rng = rand::thread_rng();

    for i in 0..50 {
        let amount_b_raw = rng.gen_range(1..=1000);
        let amount_a: u64 = format_value_with_decimals(rng.gen_range(1..=1000), 6);
        let amount_b: u64 = format_value_with_decimals(amount_b_raw, 6);
        let amount_b_consumer: u64 = format_value_with_decimals(rng.gen_range(1..=amount_b_raw), 6);

        println!("Running Fuzz Test #{:?}", i);
        println!("Amount A: {:?}", amount_a);
        println!("Amount B: {:?}", amount_b);
        println!("Amount B Consumer: {:?}", amount_b_consumer);
        ...
        // SWAPp note consumer wallet balance
        let swap_consumer_balance_token_b = format_value_with_decimals(amount_b_consumer, 8);
        let swap_consumer_token_b = FungibleAsset::new(faucet_id_2, swap_consumer_balance_token_b)
            .unwrap()
            .into();
        let (target_pub_key, _target_falcon_auth) = get_new_pk_and_authenticator();
        ...
        println!("Test passed for iteration: {:?}", i);
    }
}
 */
