...
pub fn create_partial_swap_note(
    ...
    let note_code = include_str!("../../src/test/p2id_recipient_check.masm");
    ...
    println!("SCRIPT HASH: {:?}", payback_recipient.script().hash());
    println!("input hash: {:?}", payback_recipient.inputs().commitment());
    println!("serial num: {:?}", serial_num);
    println!("payback recipient word: {:?}", payback_recipient_word);
